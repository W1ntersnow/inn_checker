...
dataInput.addEventListener("keypress", event => {
  ...
  if(event.keyCode === 13 && dataInput.value){
    channel.push("new_msg", {body: dataInput.value})
    dataInput.value = ""
  }
})
...
channel.on("new_msg", payload => {
  ...
  dataItem.innerText = `[${payload.dt}] ${payload.inn} : ${payload.is_correct}`
  dataContainer.prepend(dataItem) 
})
